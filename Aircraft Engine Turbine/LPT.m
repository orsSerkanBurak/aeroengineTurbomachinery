clc;clear; close all;
%% LPT Aerodynamic Calculations

alpha2 = -80:0.1:80; % Stage turning angle
M3R = 0.7; % Relative Mach number
M2 = 0.9; % Stage exit Mach number
gamma_t = 1.3; % Ratio of specific heats
omega = 0.42; % Dimensionless turbine rotor speed
...
grid on;
title('Stage total temperature ratio at \Omega = 0.3449, M_{2} = 0.9, M_{3R} = 0.7 and \gamma_{t} = 1.3');

figure(2)
plot(alpha2,Beta,'LineWidth',1.2);
xlabel('\alpha_{2} (deg)'); ylabel('\beta_{2}+\beta_{3} (deg)');
grid on;
title('Rotor flow turning angle at \Omega = 0.3449, M_{2} = 0.9, M_{3R} = 0.7 and \gamma_{t} = 1.3');

figure(3)
plot(alpha2,alpha3,'LineWidth',1.2);
xlabel('\alpha_{2} (deg)'); ylabel('\alpha_{3} (deg)');
grid on;
title('Stage exit flow angle at \Omega = 0.3449, M_{2} = 0.9, M_{3R} = 0.7 and \gamma_{t} = 1.3');

figure(4)
plot(alpha2,solidity,'LineWidth',1.2);
xlabel('\alpha_{2} (deg)'); ylabel('\sigma_{xr}');
grid on;
title('Rotor solidity based on Z=1 at \Omega = 0.3449, M_{2} = 0.9, M_{3R} = 0.7 and \gamma_{t} = 1.3');

figure(5)
plot(alpha2,Rt,'LineWidth',1.2);
xlabel('\alpha_{2} (deg)'); ylabel('R_{t}');
grid on;
title('Rotor degree of reaction at \Omega = 0.3449, M_{2} = 0.9, M_{3R} = 0.7 and \gamma_{t} = 1.3');
axis([-80 80 -1 1.5]);

%choosing parameters due to chosen angle1
%for i = 1:1:18001
%if alpha1(i) == 28
%end
%end